...
function listRecords() {
  return fs
    .readdirSync(JSX_DIR, { withFileTypes: true })
    .filter((x) => x.isFile() && allowed(x.name))
    .map((x) => {
      const full = path.join(JSX_DIR, x.name),
        st = fs.statSync(full);
      return {
        id: fileId(x.name),
        filename: x.name,
...
function getRecord(id) {
  ...
  if (!name) return null;
  const full = path.join(JSX_DIR, name);
...
function uniquePath(filename) {
  const base = safe(filename),
    ext = path.extname(base),
    stem = path.basename(base, ext);
  let name = base,
    n = 2;
  while (fs.existsSync(path.join(JSX_DIR, name)))
    name = `${stem} (${n++})${ext}`;
...
function plugins(record) {
  return [
    {
      name: "viewer-resolver",
      setup(build) {
        build.onResolve({ filter: /^@\/components\/ui\// }, (a) => ({
          path: a.path.split("/").pop(),
          namespace: "shadcn",
        }));
        build.onLoad({ filter: /.*/, namespace: "shadcn" }, (a) => ({
          contents:
            STUBS[a.path] ||
            `import React from 'react'; export default function X({children,...p}){return <div {...p}>{children}</div>}`,
          loader: "jsx",
        }));
        ...
        build.onLoad({ filter: /.*/, namespace: "util" }, () => ({
          contents: `export function cn(...x){return x.flat().filter(Boolean).join(' ')}`,
          loader: "js",
        }));
        build.onResolve({ filter: /^[^./][^:]*/ }, (a) => {
          try {
            return { path: requireFromViewer.resolve(a.path) };
          } catch {
            return null;
          }
        });
        build.onResolve({ filter: /^\.\.?\// }, (a) =>
          a.namespace === "target"
            ? { path: a.path, namespace: "missing-relative" }
            : null,
        );
        build.onLoad({ filter: /.*/, namespace: "missing-relative" }, (a) => ({
          errors: [
            {
              text: `単体アップロードでは相対 import (${a.path}) を解決できません。`,
            },
          ],
        }));
      },
...
async function bundle(record) {
  const uploaded = {
    name: "uploaded",
    setup(build) {
      build.onResolve({ filter: /^__TARGET__$/ }, () => ({
        path: record.filename,
        namespace: "target",
      }));
      build.onLoad({ filter: /.*/, namespace: "target" }, () => ({
        contents: record.source,
        loader: record.filename.endsWith(".tsx")
          ? "tsx"
          : record.filename.endsWith(".ts")
            ? "ts"
            : "jsx",
  ...
  const result = await esbuild.build({
    ...
    plugins: [uploaded, ...plugins(record)],
    logLevel: "silent",
  });
  const f =
    result.outputFiles.find((x) => x.path.endsWith("bundle.js")) ||
    result.outputFiles.find((x) => x.path.endsWith(".js"));
  if (!f) throw new Error("bundle.js を生成できませんでした");
  return f.text;
}
async function css(record) {
  const input =
    "@tailwind base;@tailwind components;@tailwind utilities;html,body,#root{min-height:100%}body{margin:0}*{box-sizing:border-box}";
  const cfg = {
    content: [
      {
        raw: record.source,
        extension: record.filename.split(".").pop() || "jsx",
      },
      { raw: Object.values(STUBS).join("\n"), extension: "jsx" },
    ],
    theme: { extend: {} },
    plugins: [],
  };
  return (
    await postcss([tailwindcss(cfg), autoprefixer]).process(input, {
      from: undefined,
    })
  ).css;
}
...
function previewPage(r) {
  return `<!doctype html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(r.filename)} - JSX Viewer</title><style>html,body{height:100%;margin:0}body{display:flex;flex-direction:column;background:#fff;font-family:system-ui,-apple-system,"Segoe UI",sans-serif}#bar{display:flex;gap:8px;align-items:center;padding:7px 10px;background:#111827;color:white;font:13px system-ui}#bar .n{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}#bar a,#bar button{border:1px solid #4b5563;border-radius:7px;padding:6px 9px;background:#1f2937;color:white;text-decoration:none;cursor:pointer}#wrap{flex:1;min-height:0}#preview{display:block;width:100%;height:100%;border:0;background:white}</style></head><body><div id="bar"><a href="/">一覧</a><strong class="n">${esc(r.filename)}</strong><button onclick="document.getElementById('preview').src='/sandbox?id=${r.id}&t='+Date.now()">再読み込み</button></div><div id="wrap"><iframe id="preview" src="/sandbox?id=${r.id}" sandbox="allow-scripts" referrerpolicy="no-referrer" title="${esc(r.filename)}"></iframe></div></body></html>`;
}

function sandboxPage(r, css, js) {
  const safeJs = String(js).replace(/<\/script/gi, "<\\/script");
  return `<!doctype html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(r.filename)}</title><style>${css}</style><style>html,body,#root{width:100%;min-height:100%;min-width:0;max-width:100%}body{margin:0;overflow-x:hidden}#root{overflow-x:hidden}#root>*{max-width:100%}*,*::before,*::after{box-sizing:border-box;min-width:0}img,svg,video,canvas{max-width:100%;height:auto}button,input,textarea,select{max-width:100%}nav>div{max-width:100%;overflow-x:auto;overflow-y:hidden}nav button{flex:0 0 auto;white-space:nowrap}@media(max-width:600px){[style*="max-width"]{max-width:100%!important}p,h1,h2,h3,h4,h5,h6,span,div{overflow-wrap:anywhere}}#err{display:none;white-space:pre-wrap;margin:16px;padding:16px;background:#fef2f2;color:#991b1b;border:1px solid #fecaca;border-radius:8px;font:13px/1.5 Consolas,monospace}</style></head><body><div id="err"></div><div id="root"></div><script>function showError(p,v){const b=document.getElementById('err');b.style.display='block';b.textContent=p+'\\n'+(v?.stack||v?.message||String(v))}window.addEventListener('error',e=>showError('実行時エラー:',e.error||e.message));window.addEventListener('unhandledrejection',e=>showError('Promise エラー:',e.reason));</script><script>${safeJs}</script></body></html>`;
}
  ...
  try {
    ...
    if (req.method === "GET" && u.pathname === "/api/files")
      return json(
        res,
        200,
        listRecords().map(({ source, ...x }) => x),
      );
    if (req.method === "POST" && u.pathname === "/api/upload") {
      const b = await readBody(req),
        fn = String(b.filename || ""),
        src = String(b.source || "");
      if (!/\.(jsx|tsx|js|ts)$/i.test(fn))
        return json(res, 400, { error: "JSX / TSX / JS / TS のみ対応" });
      if (!src.trim()) return json(res, 400, { error: "空ファイルです" });
      const r = saveRecord(fn, src);
      return json(res, 200, { id: r.id, filename: r.filename });
    }
    if (req.method === "DELETE" && u.pathname.startsWith("/api/files/")) {
      const id = u.pathname.split("/").pop();
      return json(res, 200, { ok: deleteRecord(id) });
    }
    if (req.method === "GET" && u.pathname === "/preview") {
      const r = getRecord(u.searchParams.get("id"));
      if (!r) {
        res.writeHead(404);
        return res.end("Not found");
      }
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      return res.end(previewPage(r));
    }
    if (req.method === "GET" && u.pathname === "/sandbox") {
      const r = getRecord(u.searchParams.get("id"));
      if (!r) {
        res.writeHead(404);
        return res.end("Not found");
      ...
      try {
        const [stylesheet, js] = await Promise.all([css(r), bundle(r)]);
        res.writeHead(200, {
          "Content-Type": "text/html; charset=utf-8",
          "Cache-Control": "no-store",
          "Content-Security-Policy":
            "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; font-src data:; connect-src 'none'; media-src 'none'; object-src 'none'; frame-src 'none'; worker-src 'none'; form-action 'none'; base-uri 'none';",
          "Referrer-Policy": "no-referrer",
          "X-Content-Type-Options": "nosniff",
        });
        return res.end(sandboxPage(r, stylesheet, js));
      } catch (err) {
        const details =
          err?.errors
            ?.map(
        ...
        return res.end(
          `<!doctype html><meta charset="UTF-8"><style>body{font-family:Consolas,monospace;padding:16px;color:#991b1b;background:#fef2f2;white-space:pre-wrap}</style>ビルドエラー:\n${esc(details)}`,
        );
      }
    }
    if (req.method === "GET" && u.pathname === "/bundle.js") {
      ...
      try {
          ...
          e?.errors
            ?.map(
              (x) =>
                x.text +
                (x.location
                  ? `\n${x.location.file}:${x.location.line}:${x.location.column}`
                  : ""),
            )
            .join("\n\n") ||
          e.stack ||
        ...
        return res.end(`showError('ビルドエラー:',${JSON.stringify(d)});`);
      }
    }
  ...
  for (const es of Object.values(os.networkInterfaces()))
    for (const x of es || [])
      if (x.family === "IPv4" && !x.internal)
        a.push(`http://${x.address}:${PORT}/`);
  return [...new Set(a)];
}
server.listen(PORT, HOST, () => {
  console.log("\n==========================================");
  console.log(" Claude JSX Viewer Server v2.4");
  console.log("==========================================");
  console.log(`PC     : http://127.0.0.1:${PORT}/`);
  for (const u of lans()) console.log(`LAN    : ${u}`);
  console.log(
    "\nスマホを同じWi-Fi/LANに接続し、LAN URLへアクセスしてください。",
  );
  console.log("このウィンドウを閉じるとサーバも停止します。\n");
